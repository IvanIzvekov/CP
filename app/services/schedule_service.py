...
def parse_excel_sheets(file_bytes: bytes) -> Dict[str, Dict[any, any]]:
    ...
    with pd.ExcelFile(io.BytesIO(file_bytes)) as xls:
        df_sheet_settings = pd.read_excel(
            xls, sheet_name="Настройка графика", header=None
        )
        df_sheet_vigils_summer = pd.read_excel(
            xls, sheet_name="Календарь нарядов (ЛЕТ)", header=None
        )
        df_sheet_vigils_winter = pd.read_excel(
            xls, sheet_name="Календарь нарядов (ЗИМ)", header=None
        )
    ...
    try:
        ...
        matches = (
            df_sheet_settings[
                df_sheet_settings.eq("Начальная дата календаря нарядов")
            ]
            .stack()
            .index
        )
        if len(matches) == 0:
            raise ExcelParsingError(
                "Не найдена ячейка 'Начальная дата календаря нарядов' на листе 'Настройка графика'"
            )
        start_pos = matches[0]
        scheduler_start_at = safe_iat(
            df_sheet_settings, start_pos[0] + 1, start_pos[1]
        )
        if scheduler_start_at is None or pd.isna(scheduler_start_at):
            raise ExcelParsingError(
                "Начальная дата календаря нарядов не найдена или пуста (лист 'Настройка графика')"
            )

        # ---- второе вхождение для "Расписание ГК", т.к. первое вхождение содержит строку со всеми датами (неудобно парсить)----
        matches_gk = (
            df_sheet_settings[df_sheet_settings.eq("Расписание ГК")]
            .stack()
            .index
        )
        if len(matches_gk) < 2:
            raise ExcelParsingError(
                "Не найдено второе вхождение 'Расписание ГК' (лист 'Настройка графика')"
            )
        gk_pos = matches_gk[1]
        ...
        for offset in range(0, 10):
            val = safe_iat(
                df_sheet_settings, gk_pos[0] + 1, gk_pos[1] + offset
            )
        ...
        matches_resp = (
            df_sheet_vigils_summer[
                df_sheet_vigils_summer.eq("График ответственных")
            ]
            .stack()
            .index
        )
        if len(matches_resp) == 0:
            raise ExcelParsingError(
                "Не найдена ячейка 'График ответственных' (лист 'Календарь нарядов (ЛЕТ)')"
            )
        ...
        for offset in range(1, n_cols_s - resp_pos[1]):
            ...
            if val is None or pd.isna(val):
                break
            scheduler_responsible[
                scheduler_current_date.strftime("%d-%m-%Y")
            ] = val
            scheduler_current_date += datetime.timedelta(days=1)

        # ---- Сбор ФИО -> должность (fios_post) из summer и winter ----
        matches_fio = (
            df_sheet_vigils_summer[df_sheet_vigils_summer.eq("ФИО")]
            .stack()
            .index
        )
        if len(matches_fio) == 0:
            raise ExcelParsingError(
                "Не найдена ячейка 'ФИО' (лист 'Календарь нарядов (ЛЕТ)')"
            )
        ...
        matches_start_col = (
            df_sheet_vigils_summer[
                df_sheet_vigils_summer.eq(scheduler_start_at)
            ]
            .stack()
            .index
        )
        if len(matches_start_col) == 0:
            raise ExcelParsingError(
                "Не найдена ячейка с датой старта в 'Календарь нарядов (ЛЕТ)'"
            )
        ...
        summer_df = df_sheet_vigils_summer.iloc[
            summer_row_start:summer_row_end, summer_col_start:summer_col_end
        ]
        ...
        summer_index = [
            safe_iat(df_sheet_vigils_summer, r, fio_pos[1])
        ...
        winter_df = df_sheet_vigils_winter.iloc[
            winter_row_start:winter_row_end, winter_col_start:winter_col__end
        ]
        ...
        winter_index = [
            safe_iat(df_sheet_vigils_winter, r, fio_pos[1])
        ...
        if summer_dates != winter_dates:
            raise ExcelParsingError(
                "Наборы дат в летнем и зимнем расписаниях не совпадают!"
            )
        ...
        raise ExcelParsingError(
            f"Таблица графиков была изменена, ошибка чтения данных: {e}"
        )
...
class ScheduleService:
    ...
    async def get_vigils(self, **kwargs) -> List[ScheduleVigilEntity]:
        resp_vigil_id = await self.schedule_repo.get_vigils_type(
            name=["Ответственный"]
        )
        if resp_vigil_id:
    ...
    async def create_vigils(
        self, info: Dict[str, Dict[str, str]], users: list[UserEntity]
    ):
        ...
        if vigils:
            ...
            vigils_min_date = (
                min(all_vigils_dates) if all_vigils_dates else None
            )
            vigils_max_date = (
                max(all_vigils_dates) if all_vigils_dates else None
            )

        else:
            raise NoVigilsDataFromExcel(
                "Нет данных об нарядах в таблице (либо данные ошибочные), проверьте, что всем именам соответствуют реальные пользователи в системе"
            )
        ...
        if responsible:
            ...
            if not filtered_responsible:
                raise NoVigilsDataFromExcel(
                    "Нет данных об ответственных в таблице (либо данные ошибочные), проверьте, что всем именам соответствуют реальные пользователи в системе"
                )
        ...
        await self.schedule_repo.save_group_control_schedule(
            group_control_schedule=group_control,
            start_date=gc_min_date,
            end_date=gc_max_date,
        )
        await self.schedule_repo.save_responsible_schedule(
            responsible=filtered_responsible,
            start_date=resp_min_date,
            end_date=resp_max_date,
        )
        await self.schedule_repo.save_vigils_schedule(
            vigils_schedule=vigils_by_id,
            start_date=vigils_min_date,
            end_date=vigils_max_date,
        )

    async def process_vigils_schedule(
        ...
        try:
            with ProcessPoolExecutor(max_workers=2) as executor:
                result = await asyncio.wait_for(
                    loop.run_in_executor(
                        executor, parse_excel_sheets, file_bytes
                    ),
                    timeout=10,
                )
                return result
            ...
            raise ExcelParsingError(
                "Парсинг Excel завис, возможно файл нестандартный, проверьте файл и попробуйте позже"
            )
