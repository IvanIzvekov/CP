...
@router.get("/update_database_config", description="Вносит данные в справочники. Запуск возможен только если таблицы пустые")
async def update_data_config(session=Depends(get_session)):
    ...
    try:
        service = UtilsService(user_repo=UserRepository(session), schedule_repo=ScheduleRepository(session))
        user_service = UserService(UserRepository(session))
        exist_superuser = await service.get_superusers()
        if exist_superuser:
            raise HTTPException(status_code=400, detail="Суперюзер уже существует, запуская данный метод вы можете нарушить работу всей системы так как сотрете данные справочников.")

        vigil = [VigilEnumEntity(name="Дежурный по роте", is_deleted=False, name_in_csv="Р", post_in_csv="ДЖ"),
                 VigilEnumEntity(name="Дневальный по роте", is_deleted=False, name_in_csv="Р", post_in_csv="ДН"),
                 VigilEnumEntity(name="Заступающий дежурный по роте", is_deleted=False, name_in_csv="ЗН Р", post_in_csv="ДЖ"),
                 VigilEnumEntity(name="Заступающий дневальный по роте", is_deleted=False, name_in_csv="ЗН Р", post_in_csv="ДН"),
                 VigilEnumEntity(name="Ответственный", is_deleted=False, name_in_csv="Ответственный", post_in_csv="Ответственный"),
                 VigilEnumEntity(name="Рабочая группа", is_deleted=False, name_in_csv="РГ", post_in_csv="РГ"),
                 VigilEnumEntity(name="Дежурный по технопарку", is_deleted=False, name_in_csv="ДТП", post_in_csv="ДТП"),
                 VigilEnumEntity(name="Заступающий дежурный по технопарку", is_deleted=False, name_in_csv="ЗН ДТП", post_in_csv="ЗН ДТП")]

        vigils_type = await service.set_standard_vigils(data=vigil)
        company_duties = [CompanyDutyEntity(name="Каптер"),
        ...
        ranks = [RankEntity(name="Рядовой", short_name="ряд."),
        ...
        posts = [PostEntity(name="Оператор роты (научной)"),
