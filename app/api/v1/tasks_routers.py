...
async def task_create(
    ...
    try:
        task = await service.create(responsible_id=user.id,
                                    text=data.text,
                                    owner_id=user.id,
...
async def user_task_create(
    data: TaskCreate,
    responsible_type: Literal["user", "duty"] = Path(..., description="Тип ответственного: user или duty"),
    responsible_id: UUID = Path(..., description="ID ответственного (пользователь или группа по обязанности)"),
    session: AsyncSession = Depends(get_session),
    user: UserEntity = Depends(check_auth_dep),
):
    pass
